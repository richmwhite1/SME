-- =====================================================
-- Enhanced Global Search RPC Function
-- =====================================================
-- Upgrades global_search to include AI summaries for contextual search
-- Returns snippets from AI summaries/reviews that match the query
-- =====================================================
...
BEGIN
  ...
  product_results AS (
    SELECT
      ...
      CASE
        WHEN pr.ai_summary IS NOT NULL AND lower(pr.ai_summary) LIKE '%' || lower(search_query) || '%' THEN
          substring(
            pr.ai_summary,
            greatest(1, position(lower(search_query) in lower(pr.ai_summary)) - 50),
      ...
      CASE
        ...
        WHEN pr.ai_summary IS NOT NULL AND lower(pr.ai_summary) LIKE '%' || lower(search_query) || '%' THEN 8
    ...
    WHERE (
      ...
      OR (pr.ai_summary IS NOT NULL AND lower(pr.ai_summary) LIKE '%' || lower(search_query) || '%')
    )
  ...
  discussion_results AS (
    SELECT
      ...
      CASE
        WHEN lower(d.content) LIKE '%' || lower(search_query) || '%' THEN
          substring(
            d.content,
            greatest(1, position(lower(search_query) in lower(d.content)) - 50),
...
-- Add comment
COMMENT ON FUNCTION global_search IS 'Enhanced search across protocols, discussions, and resource_library. Includes AI summary snippets for contextual search.';
