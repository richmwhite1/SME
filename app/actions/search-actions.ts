 ...
 */
import { deepSearch, DeepSearchSummary } from "@/lib/ai/deep-search";

// ... existing imports

export interface SearchResponse {
  results: SearchResult[];
  synthesis?: DeepSearchSummary | null; // Structured AI summary
}
...
export async function searchGlobal(
  ...
  // --- AI Synthesis Layer ---
  let synthesis: DeepSearchSummary | null = null;
  ...
  if ((isQuestion || results.length > 0) && results.length > 0) {
    try {
      synthesis = await deepSearch(query, results);
    } catch (e) {
