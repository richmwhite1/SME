...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  if (filter === "certified") {
    filteredResults = results.filter((r) => r.is_sme_certified);
  } else if (filter === "products") {
    filteredResults = results.filter((r) => r.result_type === "Product");
  } else if (filter === "discussions") {
    filteredResults = results.filter((r) => r.result_type === "Discussion");
  } else if (filter === "resources") {
    filteredResults = results.filter((r) => r.result_type === "Resource" || r.result_type === "Evidence");
  }
            ...
            <div className="sticky top-24">
              <div className="border border-translucent-emerald bg-muted-moss p-4">
                ...
                <Suspense fallback={<div className="text-xs text-bone-white/70 font-mono">Loading filters...</div>}>
                  <SearchFilters currentFilter={filter} query={query} />
                </Suspense>
              </div>
            </div>
          </aside>
